...
function ScriptReviewContent() {
  ...
  const handleCreateVideo = async () => {
    ...
    setVideoState(prev => ({ ...prev, isGenerating: true, error: null }));
    ...
    try {
      ...
      const numScenes = scriptData.image_data?.scene_images.length || 0;
      
      // Create arrays for effects
      const randomClipEffects: string[] = [];
      const randomTransitions: string[] = [];
      
      // Shuffle arrays to ensure unique combinations
      const shuffledClipEffects = [...clipEffects].sort(() => Math.random() - 0.5);
      const shuffledTransitions = [...transitionEffects].sort(() => Math.random() - 0.5);
      
      // Assign unique clip effects to each scene
      for (let i = 0; i < numScenes; i++) {
        randomClipEffects.push(shuffledClipEffects[i % shuffledClipEffects.length]);
      }
      
      // Assign unique transition effects between scenes
      for (let i = 0; i < numScenes - 1; i++) {
        randomTransitions.push(shuffledTransitions[i % shuffledTransitions.length]);
      }

      // Create the request payload
      const payload = {
        visual_urls: scriptData.image_data?.scene_images.map(img => img.cloudinary_url) || [],
        audio_urls: scriptData.voice_data?.scene_voiceovers.map(vo => vo.cloudinary_url) || [],
        effects: randomClipEffects, // Unique clip effects for each scene
        transitions: randomTransitions, // Unique transitions between scenes
        background_music_url: null,
        metadata: {
          num_scenes: scriptData.image_data?.scene_images.length || 0,
          num_audio_segments: scriptData.voice_data?.scene_voiceovers.length || 0
        }
      };

      // Call the deployed video-gen service
      const response = await fetch('https://tkpm.up.railway.app/video-assembly/videos', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
